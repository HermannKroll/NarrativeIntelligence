...
class PublicationDateRank(DocumentRanker):

    def rank_document(self, graph_query: GraphQuery, documents: List[QueryDocumentResult], desc: bool = True) -> List[
        QueryDocumentResult]:
        return sorted(documents, key=lambda x: (x.publication_year, x.publication_month), reverse=desc)
...
class GraphRank:
    ...
    def rank_document(self, graph_query: GraphQuery, documents: List[QueryDocumentResult], desc: bool = True) -> List[
        ...
        for collection in collections:
            dids = {r.document_id for r in documents if r.document_collection == collection}
        ...
        return sorted(documents, key=lambda x: (x.score, x.document_id), reverse=desc)
