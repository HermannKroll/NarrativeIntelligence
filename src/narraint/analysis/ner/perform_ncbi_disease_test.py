        ...
        with open(VOCAB_FILE, newline='') as f:
            ...
            for row in vocab_reader:
                ...
                if len(row[0]) < tagger.config.dict_min_full_tag_len:
                    continue
                names = [name for n in [row[0]] + row[7].split("|") for name in expand_vocabulary_term(n.lower()) if
                         len(n) >= min_len and n.lower() not in stopwords and name not in stopwords]
...
def run_test(s_e_tolerance: int = 1):
        ...
        if '|' in tag.ent_id:
            for ent_id in tag.ent_id.split('|'):
                correct_diseases[int(tag.document)].add(
                    TaggedEntity(document=tag.document, start=tag.start, end=tag.end, text=tag.text,
            ...
            for tag in correct_tags:
                ...
                if not SUBSTRING_MATCHING:
                    ...
                    for compare_tag in tagged_list:
                        if tag.ent_id == compare_tag[-1] and compare_tag[0] + 1 >= tag.start and tag.end >= compare_tag[
                            1]:
                            matched = True
                            break
                    else:
    ...
    precision = count_correct_extractions / (count_correct_extractions + count_wrong_extractions) if (
                                                                                                                 count_correct_extractions + count_missing_extractions) > 0 else 0
    recall = count_correct_extractions / (count_correct_extractions + count_missing_extractions) if (
                                                                                                                count_correct_extractions + count_missing_extractions) > 0 else 0
