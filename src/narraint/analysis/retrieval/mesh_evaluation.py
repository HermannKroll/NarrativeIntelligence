...
metabolises_enhanced_search = (
metabolises_subject_qualifiers, metabolises_object_qualifiers, metabolises_additional_descriptors)
...
decreases_enhanced_search = (
decreases_subject_qualifiers, decreases_object_qualifiers, decreases_additional_descriptors)
        ...
        for p, p_s_types, p_o_types in query_predicates:
            ...
            for extraction_type in [OPENIE_EXTRACTION, PATHIE_EXTRACTION]:
                ...
                with open(filename, 'wt') as f:
                    ...
                    logging.info(
                        'Query subjects and objects for predicate: {} ({} -> {})'.format(p, p_s_types, p_o_types))
                    pred_subjects_and_objects = get_subject_object_for_predicate(p, extraction_type, p_s_types,
                                                                                 p_o_types)
                    ...
                    for idx, (sub_id, sub_type, obj_id, obj_type) in enumerate(pred_subjects_and_objects):
                        ids_correct = pubmed_mesh_hits(None, sub_id, p, obj_id,
                                                       compute_subdescriptors=DO_QUERY_MESH_EXPANSION)
                        if obj_type == GENE:
                        ...
                        line_to_write = '\n{}\t{}\t{}\t{}\t{}\t{}'.format(sub_id, obj_id, len(ids_correct), hits,
                                                                          precision, recall)
