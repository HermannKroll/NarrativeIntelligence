...
## Load Everything
```
python3 ~/NarrativeIntelligence/lib/KGExtractionToolbox/src/kgextractiontoolbox/document/load_document.py trip_click_dataset.json -c TRIP_CLICK 
```

# Next, tag the documents with our PharmDictTagger
```
python3 ~/NarrativeIntelligence/lib/NarrativeAnnotation/src/narrant/preprocessing/dictpreprocess.py -c TRIP_CLICK --workers 15
```

# Next, tag the documents with GNormPlus
```
python3 ~/NarrativeIntelligence/lib/KGExtractionToolbox/src/kgextractiontoolbox/entitylinking/biomedical_entity_linking.py trip_click_dataset.json -c TRIP_CLICK --skip-load --workers 15 --gnormplus
```
