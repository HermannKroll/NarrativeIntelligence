...
class QueryLogger:
    ...
    def __init__(self, log_dir=LOG_DIR):
        ...
        self.log_dir_doc_classification = os.path.join(log_dir, 'document_classification')
        if not os.path.isdir(log_dir):
        ...
        if not os.path.isdir(self.log_dir_doc_classification):
            os.mkdir(self.log_dir_doc_classification)
        ...
        self.doc_classification_header = 'timestamp\tdocument id\tcollection\tclassification\trating'
    ...
    def write_document_classification(self, document_id, document_collection, classification, rating):
        log_file_name = os.path.join(self.log_dir_doc_classification,
                                     f'{time.strftime("%Y-%m-%d")}-document_classification.log')
        log_entry = f'{document_id}\t{document_collection}\t{classification}\t{rating}'
        write_entry(log_entry, log_file_name, self.doc_classification_header, "doc classification feedback")