...
async function getOverviewData() {
    for (let prefix in overviews) {
        ...
        const url = `${url_query_sub_count}?query=${LONG_COVID_ENTITY}+${ov.predicate}+${ov.object}&data_source=PubMed`;
...
async function fillSearchbox(prefix) {
    ...
    for (let i = overviews[prefix].numVisible - VISIBLE_ELEMENTS; i < maxLen; i++) {
        ...
        const element =
            `<a href=${query} style="text-decoration: none; color: inherit;" onclick="logSubstanceHref('${LONG_COVID_ENTITY}','${item.name}','${stringQuery}')" target="_blank">
                <p>${item.name}</p>
            </a>
            <div style="background-color: ${bgColor};" class="count">
                <a href=${query} target="_blank" onclick="logSubstanceHref('${LONG_COVID_ENTITY}','${item.name}','${stringQuery}')">${item.count}</a>
            </div>`;
...
function getLinkToQuery(reference, item) {
    const subject = LONG_COVID_ENTITY.split(' ').join('+');
...
function fillNews(data) {
    ...
    if (i > 0) {
        ...
        morePaper.href =
            `https://www.pubpharm.de/vufind/Search/Results?lookfor=${LONG_COVID_ENTITY}&type=AllFields`;
...
async function load_wordcloud() {
    const data = await fetch(url_keywords + "?substance_id=" + LONG_COVID_ENTITY_ID)
        .then(async (response) => {
        ...
        .catch(() => {
            console.log("No keywords for " + LONG_COVID_ENTITY_ID + "available.");
            return null;
        });
...
function updateNetworkGraph(firstInit = false) {
    ...
    // root node
    nodes.add({id: 1, label: LONG_COVID_ENTITY, group: "drugAssociationNode"})
...
const networkSelectEdgeCallback = (e, network) => {
    ...
    window.open(`/?query="${LONG_COVID_ENTITY}"+${predicate}+"${object}"`, '_blank');
    network.unselectAll();
}
